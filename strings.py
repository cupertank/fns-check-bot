class Strings:
    HELP = """<b> Бот для разделения чеков ФНС 🧾🇷🇺</b>
    
    <b>🤖 Что умеет этот бот?</b>
    С помощью этого бота вы можете разделить чек без долгих подсчётов на калькуляторе! 🤓
    
    <b>⁉ Как пользоваться ботом?</b>
    🏦  Авторизируйтесь в ФНС по номеру телефона.
    👩🏼🧑👩🏿👳  Выберите, между кем следует разделить счёт.
    📸🧾 Отправьте боту фото чека, чтобы QR код был хорошо виден.
    💸 Определите для каждой позиции, кто за это должен заплатить.
    ...
    Подробная информация о боте доступна в /help. ℹ️
    
    Пожалуйста, введи свой номер телефона 📞, чтобы авторизироваться в ФНС:
    """

    EnterNames = """👩🏼🧑👩🏿👳  Введи имена или ники друзей, между которыми ты хочешь разделить чек, по одному на каждой 
    строке. Например: 
    ...
    FNSError = "💥  На сервере ФНС что-то пошло не так... Пожалуйста, попробуй, авторизоваться заново: /login. "
